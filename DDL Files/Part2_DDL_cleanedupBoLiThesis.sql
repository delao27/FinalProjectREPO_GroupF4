...
connOracleNoSQL = connectTo 'OracleNoSQL' 'WDBF4' 'localhost:8061' 'native_mode' 'A0' 
SIM on connOracleNoSQL "CLEAR DATABASE"

# Organization Schema
#Person
SIM on connOracleNoSQL '''CLASS Person(person-id : INTEGER, REQUIRED; first-name : STRING, REQUIRED; last-name : STRING, REQUIRED; home_address : STRING; zipcode : INTEGER; home-phone "Home phone number (optional)" : INTEGER; us-citizen "U.S. citizenship status" : BOOLEAN, REQUIRED; spouse "Persons spouse if married" : Person, INVERSE IS spouse; children "Persons children (optional)" : Person, MV(DISTINCT), INVERSE IS parents; parents "Persons parents (optional)" : Person, MV (DISTINCT, MAX 2), INVERSE IS children; );'''
...
#Employee
SIM on connOracleNoSQL '''SUBCLASS Employee "Current employees of the company" OF Person(employee-id "Unique employee identification" : INTEGER, REQUIRED; salary "Current yearly salary" : INTEGER, REQUIRED; salary-exception "TRUE if salary can exceed maximum" : BOOLEAN; employee-manager "Employee's current manager" : Manager, INVERSE IS employees-managing;);'''
...
# ProjectEmployee
SIM on connOracleNoSQL '''SUBCLASS ProjectEmployee "Employees who are project team members" OF Employee (CurrentProjects "current project of employee" : CurrentProject, MV (DISTINCT, MAX 6), INVERSE IS project-members;);'''
# Person with Person-id 1, 2, 5, 6 and 7 made ProjectEmployee
SIM on connOracleNoSQL 'INSERT ProjectEmployee FROM Employee WHERE employee-id = 101 ();'
SIM on connOracleNoSQL 'INSERT ProjectEmployee FROM Employee WHERE employee-id = 102 ();'
SIM on connOracleNoSQL 'INSERT ProjectEmployee FROM Person WHERE person-id = 3 (employee-id:= 103,salary:= 80210) ;'
SIM on connOracleNoSQL 'INSERT ProjectEmployee FROM Employee WHERE employee-id = 106 ();'
SIM on connOracleNoSQL 'INSERT ProjectEmployee FROM Employee WHERE employee-id = 107 ();'
# Finally person-id 1, 2, 3, 6 and 7 are ProjectEmployee

#Manager
SIM on connOracleNoSQL '''SUBCLASS Manager "Managers of the company" OF Employee ( bonus "Yearly bonus, if any" : INTEGER; employees-managing "Employees reporting to manager" : Employee, MV, INVERSE IS employee-manager; projects-managing "Projects responsible for" : Project, MV, INVERSE IS project-manager; manager-dept "Department to which manager belong" : Department, INVERSE IS dept-managers;);'''
...
# Finally person-id 1 , 7 and 8 are ProjectEmployee

# InterimManager
SIM on connOracleNoSQL '''SUBCLASS InterimManager "Employees temporarily acting as a project employee and a manager" OF Manager AND ProjectEmployee();'''
# Person with Person-id 1 and 7 made InterimManager.
# Note 7 will be automatically made manager
SIM on connOracleNoSQL 'INSERT InterimManager FROM Manager WHERE employee-id = 101 ();'
SIM on connOracleNoSQL 'INSERT InterimManager FROM Employee WHERE employee-id = 107 ();'
...
# PreviousEmployee
SIM on connOracleNoSQL '''SUBCLASS PreviousEmployee "Past employees of the company" OF Person ( IsFired : BOOLEAN ; salary "Salary as of termination" : INTEGER, REQUIRED;);'''
# Persons with person-id 4 created and made PreviousEmployee
SIM on connOracleNoSQL 'INSERT PreviousEmployee FROM Person WHERE person-id = 4 ( salary:= 50500 ) ;'

# Project
SIM on connOracleNoSQL '''CLASS Project "Current and completed Projects" ( project-no "Unique project identification" : INTEGER, REQUIRED; project-title "Code name for project" : STRING [20], REQUIRED; project-manager "Current project manager" : Manager, INVERSE IS projects-managing; dept-assigned "Responsible department" : Department, SV, INVERSE IS project-at; sub-projects "Component projects, if any" : Project, MV, INVERSE IS sub-project-of; sub-project-of "Master project, if any" : Project, INVERSE IS sub-projects; );'''
...
# CurrentProject
SIM on connOracleNoSQL '''SUBCLASS CurrentProject "Projects currently in progress" OF Project ( project-active "Whether project has been started" : BOOLEAN, REQUIRED; project-members "Current employees on project" : ProjectEmployee, MV (DISTINCT, MAX 20), INVERSE IS CurrentProjects; );'''
SIM on connOracleNoSQL 'INSERT CurrentProject FROM Project WHERE project-title = "Mission Impossible"( project-active := TRUE );'
SIM on connOracleNoSQL 'INSERT CurrentProject FROM Project WHERE project-title = "Hallo"( project-active := FALSE );'
SIM on connOracleNoSQL 'INSERT CurrentProject FROM Project WHERE project-title = "Stick And Fly"( project-active := TRUE );'
SIM on connOracleNoSQL 'INSERT CurrentProject FROM Project WHERE project-title = "Night Rider"( project-active := TRUE );'

# PreviousProject
SIM on connOracleNoSQL '''SUBCLASS PreviousProject "Completed Projects" OF Project ( end-date-month "Date project completed month" : INTEGER; end-date-day "Date project completed day" : INTEGER; end-date-year "Date project completed year" : INTEGER; est-person-hours "Estimated hours to complete" : INTEGER;);'''
SIM on connOracleNoSQL 'INSERT PreviousProject FROM Project WHERE project-title = "Code Red"( est-person-hours := 2000,end-date-month := 1, end-date-day := 6 , end-date-year := 1999);'
SIM on connOracleNoSQL 'INSERT PreviousProject FROM Project WHERE project-title = "Desert Rose"( est-person-hours := 1300,end-date-month := 5, end-date-day := 3 , end-date-year := 1997);'

# Department
SIM on connOracleNoSQL '''CLASS Department "Departments within the company"( dept-no "Corporate department number" : INTEGER, REQUIRED; dept-name "Corporate department name" : STRING [20], REQUIRED; project-at "Projects worked on at this department" : Project , INVERSE IS dept-assigned, MV (DISTINCT); dept-managers "Managers for this department" : Manager, MV, INVERSE IS manager-dept;);'''
...
SIM on connOracleNoSQL 'MODIFY LIMIT = ALL Employee ( employee-manager := Manager WITH(first-name = "Henry" AND last-name = "Silverstone")) WHERE employee-id = 101 OR employee-id = 107;'
SIM on connOracleNoSQL 'MODIFY LIMIT = ALL ProjectEmployee( CurrentProjects := INCLUDE CurrentProject WITH ( project-title = "Mission Impossible" )) WHERE person-id = 7 OR person-id = 3 OR person-id = 2 OR employee-id = 106 OR person-id = 1;'
SIM on connOracleNoSQL 'MODIFY LIMIT = ALL ProjectEmployee( CurrentProjects := INCLUDE CurrentProject WITH ( project-title = "Stick And Fly" )) WHERE person-id = 3 OR person-id = 7 OR person-id = 106;'
SIM on connOracleNoSQL 'MODIFY LIMIT = ALL ProjectEmployee( CurrentProjects := INCLUDE CurrentProject WITH ( project-title = "Night Rider" )) WHERE person-id = 2 OR person-id = 1 OR person-id = 7;'
